...
template<typename T>
ArkAssistant::linkList<T>::linkList()
{
	head = new Node<T>;
	tail = new Node<T>;
	head->next = tail;
...
template<typename T>
void ArkAssistant::linkList<T>::add(T e)
{
...
template<typename T>
void ArkAssistant::linkList<T>::remove(T index)
{
	if (length == 0)
		return;
	Node<T>* p = head;
...
template<typename T>
void ArkAssistant::linkList<T>::removeAll()
{
	if (length == 0)
		return;
	Node<T>* p = head->next;
...
template<typename T>
bool ArkAssistant::linkList<T>::isEmpty()
{
	return length == 0 ? true : false;
}

template <typename T>
void ArkAssistant::linkList<T>::show()
{
	...
	if (tempButton->isChecked())
	{
		for (int i = 0; i < selectCounter; i++)
		{
			if (tagSelected[i].empty())
			{
				...
				tagg.add(operatorJudge(objectNameBtn));
				tagg.show();
